...
class UFONetOptions(optparse.OptionParser):
    def __init__(self, *args):
        ...
        self.total_botnet = str(self.zombies+self.aliens+self.droids+self.ucavs)
        optparse.OptionParser.__init__(self, 
                           description='\nUFONet - DDoS Botnet via Web Abuse - by psy',
                           prog='UFONet.py',
                           version='\nVersion: v0.7 - Big Crunch!\n')
        ...
        group6.add_option("--no-head", action="store_true", dest="disablehead", help="Disable check of target's status at start")
        group6.add_option("--disable-isup", action="store_true", dest="disableisup", help="Disable round check status: 'is target up?'")
        group6.add_option("--disable-aliens", action="store_true", dest="disablealiens", help="Disable 'aliens' web abuse of test services")
        group6.add_option("--disable-droids", action="store_true", dest="disabledroids", help="Disable 'droids' redirectors")
    ...
    def get_options(self, user_args=None):
        ...
        if (not options.test and not options.target and not options.checktor and not options.search and not options.dorks and not options.inspect and not options.update and not options.download and not options.upload and not options.web and not options.attackme and not options.upip and not options.dip and not options.blackhole):
            ...
            print '='*75, "\n"
            print 'Total bots:', self.total_botnet, "= [ Z:" + str(self.zombies) + " + A:" + str(self.aliens) + " + D:" + str(self.droids) + " + U:" + str(self.ucavs) + " ]" + "\n"
            print '='*75, "\n"
